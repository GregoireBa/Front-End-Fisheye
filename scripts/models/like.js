  ...
  if (btnLike) {
    ...
    if (!likesState[mediaId]) {
      likesState[mediaId] = {
        liked: false,
        count: parseInt(
          btnLike.parentElement.querySelector(".total-likes").textContent,
          10
        ),
      };
    }
...
export function updateTotalLikes() {
  ...
  const totalLikes = Array.from(totalLikesElements).reduce(
    (acc, elem) => acc + parseInt(elem.textContent, 10),
    0
  );

  const totalLikesDisplay = document.querySelector(".photographer_likes_count");
  if (totalLikesDisplay) {
    totalLikesDisplay.innerHTML =
      '<span class="sr-only">Nombre de like total sur la page : </span>' +
      totalLikes;
  }
}
